...
bool HS071_NLP::get_nlp_info(
    ...
    m = 2;
    nnz_jac_g = 8;
    nnz_h_lag = 10; // 16 in total but hessian is symmetric so only lower left corner
...
bool HS071_NLP::get_bounds_info(
    ...
    g_l[0] = 25;
    // has no upper limit so inf is upper limit in ipopt 2e19 is the largest value
    g_u[0] = 2e19; 
...
bool HS071_NLP::get_starting_point(
    ...
    // assume initial values only for x
    assert(init_x == true);
    assert(init_z == false);
